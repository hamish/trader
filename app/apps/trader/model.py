...
class Player(db.Model):
    ...
    # Location of the player (planetId)
    location = db.IntegerProperty()
    money = db.IntegerProperty()
    products= db.ListProperty(int)
    holdSize=db.IntegerProperty()
    numProducts=db.IntegerProperty()
    
    def getProducts(self):
        if not self.products:
            self.products=[0,0,0,0,0,0,0,0]
            self.put()
    ...
    def getHoldSize(self):
        if not self.holdSize:
            self.holdSize=4
            self.put()
        return self.holdSize
    def getNumProducts(self):
        if not self.numProducts:
            self.numProducts=0
            self.put()
